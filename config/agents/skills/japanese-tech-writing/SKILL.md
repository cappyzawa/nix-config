...
description: 日本語の技術文書・書籍原稿の文章規範。整形（一文一行、引用ブロック、脚注、コラム記法）、段落と論証の構成（パラグラフライティング）、論証の厳密さ（ツッコミどころの除去）、読み手の負荷の管理、視点と語り、演出の抑制、LLM っぽい空句の禁止、冗長の排除を定める。人が読む日本語の文書（技術書の章、草稿、記事、解説文、PR/Issue 本文、docs、レビュー返信）を書くとき、または推敲・リライトするときに使用する。
---

> Source: https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d (by @k16shikano)

# 日本語技術文書の文章規範

日本語で技術的な原稿（書籍の章、記事、解説文）を書く・推敲するときは、以下の規範に従う。

書籍以外に適用するときは、書籍固有の項目（一文ごとの改行、脚注、コラム見出し、章をまたぐ前方参照、太字による術語の導入）だけを外し、残りは適用する。
...
- 地の文に (a) (b) (c) や (1) (2) (3) を連ねない。列挙は箇条書きにするか、文を分ける。
...
- 後で参照する必要のない固有名（ファイル名、関数名、識別子）を出さない。「仕様書」「金額計算のユーティリティ」のような一般的な言い方で済ませる。コードの位置も既定では書かず、どうしても必要なときだけ commit を固定した GitHub の permalink にする（`path:行番号` は更新で動き、壊れたことに読み手が気付けない）。permalink は commit hash を引く手間がかかるので、その手間に見合うかを書く前に判断する。ツールのエラー出力をそのまま引用する場合は、出力の中の行番号を残す。
...
- 手順書・runbook は例外で、節が答える問いが「どう確認するか」「どう回避するか」そのものなので、手順名の見出しでよい。
