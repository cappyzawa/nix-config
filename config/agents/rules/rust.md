...
- **誤用しにくい API を作る**: 正しい使い方が自然で、間違った使い方がコンパイルエラーになるよう設計する。
- **不変条件ごとに一番安いエンフォース手段を選ぶ**: 階段は「変更が別の力で抑止されているなら何もしない → コメント → 検証付き constructor + private repr の newtype → module / crate 境界で誤った合成を書けなくする → typestate・phantom type・型レベル機構」。下の段で足りる不変条件に上の段の型機械を使わない。重い型機構が正当化されるのは FFI 境界のように誤用コストが極端な場合だけ (dtolnay の crate 群でそれが出るのは cxx のみ)。
- **parse, don't validate**: 外界の値は境界で一度 parse して `Result` を返し、内側は「検証済み」を型 (newtype) で運ぶ。散在する再検証・到達不能な防御コード (検証後の clamp 等) は parse 点に統合して消す。繰り返し検証される対象 (スキーマ等) は使用のたびに歩き直さず、一度 parse した plan / witness 型に落とす。逆に、失敗しない total な正規化を `FromStr` など parser の形に偽装しない (「検証した」と型が嘘をつく)。
