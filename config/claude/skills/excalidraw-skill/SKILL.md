...
- Labels and text in diagrams should be written in Japanese for readability.
- Technical terms and proper nouns (e.g. Load Balancer, PostgreSQL, VPC, Kubernetes) should remain in English as-is.

## Mandatory Rules

These rules override defaults in the sections below. Always apply them.

1. **Spacing**: Gap between elements must be at least half the element width. If an element is 200px wide, the gap must be 100px+. Arrows and labels need room to coexist.
2. **Label–arrow separation**: Never place a free-standing label at the same y-coordinate as an arrow. Offset the label 15–20px above or below the arrow path.
3. **Arrow stroke width**: Always set `strokeWidth: 2` or higher. The default (1) disappears against colored backgrounds.
4. **Arrow binding**: Always use `startElementId`/`endElementId` (MCP) or `start.id`/`end.id` (REST) to bind arrows to elements. Never rely on coordinate-based arrow placement — it drifts on resize/move.
5. **No bound labels on background zones**: Use a free-standing `text` element anchored at the top-left of the zone. Bound labels center themselves and overlap contained elements.
6. **Pre-calculate canvas width**: `canvas width = element count (horizontal) × (element width + gap)`. Decide this before placing any element. Expanding later forces a full re-layout.
...
2. Optional: `curl -X DELETE http://localhost:3001/api/elements/clear`
...
```bash
curl -X POST http://localhost:3001/api/elements/batch \
...
**REST mode:**
```bash
curl -X POST http://localhost:3001/api/elements/from-mermaid \
  -H "Content-Type: application/json" \
  -d '{"mermaid": "graph TD\n  A --> B\n  B --> C"}'
```
