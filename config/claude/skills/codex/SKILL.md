...
```
codex exec -s read-only <subagent flags> --cd <project_directory> - < <prompt-file>
```

短いプロンプトは positional argument で OK:

```
codex exec -s read-only <subagent flags> --cd <project_directory> "<short request>"
```
...
2. `codex exec -s read-only <subagent flags> --cd <dir> - < <prompt-file>` を実行
...
# Step 2:
codex exec -s read-only <subagent flags> --cd <dir> - < <prompt-file>
```
...
```
codex exec -s read-only <subagent flags> --cd <dir> "<バグの症状><試した修正案 1><試した修正案 2> について、原因の仮説と次に試すべき修正方針を能動的に提案してほしい。確認や質問は不要。"
```
...
```
codex exec -s read-only <subagent flags> --cd <dir> "<質問内容>。確認や質問は不要、具体的な提案を能動的に出してほしい。"
```
...
- codex の出力に spawn / wait_agent が見える、または 1 回で数分・数百万 token 消費する → `<subagent flags>` が抜けている。`~/.codex/sessions/` の当該 rollout で `spawn_agent` を grep すると確認できる
