...
- 契約の範囲内の設計判断は指揮が決める。契約自体が誤っていたと分かったら止めてユーザーに戻す
...
ready 化・Copilot 起動・マージ・issue / PR へのコメント・人への mention は外向き操作で、**worker は指揮経由の伝聞を承認として扱えない**。
...
- Copilot ラウンド上限は変更量のルール（CLAUDE.local.md）で決め、chip に書く。人間レビュー必須の判定はプロジェクトの review policy に従い、要否と理由を chip と PR 本文に書く。仕分け（実害 / 文面磨き / 見送り）を毎ラウンド返させる
...
- worker の diff を読み込まない。読むのは worker の報告・PR 本文・CI 結果・裁定に必要な該当箇所だけ
