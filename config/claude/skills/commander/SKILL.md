...
- 調査型の chip には「仮定を置いたら計測に入る前に codex へ方法レビュー（切替手段が既定を変えないか・比較設計が症状を捉えるか・fixture の代表性・測り落とし）、修正した仮定を指揮に返してから計測」を書く。数字が出た後の方法レビューは計測のやり直しになる
...
- **スライスごとに fresh なセッションを立てる。** PR を 1 本通した worker に次のスライスを続けて渡さない。context が尽きて途中で引き継ぎになる（tierra#152 / #153 を通した後の 3 本目で実例）うえ、目が増える方が指摘が出る。token のオーバーヘッドは取る
- 動作確認の粒度を変更の大きさに合わせて chip に書く。CI の実走（一時 `pull_request` trigger 等）は 1 回 10 分超の待ちになるので、workflow に step を 1 つ足す類は静的検査 + 手元実行 + マージ後の main の 1 回で済ませ、実走を要求するのは新 job・新経路・credential に触る変更だけにする
- コメント規約と、バックグラウンドの負荷やプロセスを立てるときの `trap` を chip に書く。worker は言い換えコメントと後始末漏れをしがちで、ready 前に直す方が高い
